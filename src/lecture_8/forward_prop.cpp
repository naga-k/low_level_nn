...
int main() {
    ...
    softmax.forward(dense2.getOutput());


    Eigen::MatrixXd predictions = softmax.getOutput();
    // Output the first few samples
    std::cout << "Output of the first few samples:" << std::endl;
    std::cout << predictions.topRows(5) << std::endl;
    double loss = loss_function.calculate(predictions, y);
    std::cout << "Loss: " << loss << std::endl;
    double accuracy = metrics.accuracy(predictions, y);
